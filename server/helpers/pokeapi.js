// README: The following was used to fill up our pokemon table in our postgres database with
//         shaped data from the pokeapi.
// You will have to decide when and where to call this if your database is in need of persistant
// pokemon data.
// 1) fetchFirst151Pokemon:
//   Use this function to fill up the database with a number of pokemon from the pokeapi in
//   conjuction with savePokemon function in db.js
//   Things to consider:
//    -More than 10 requests to the pokeapi will result in a 504 timeout error (per my experience).
//    -I filled the DB by manually calling this function, each time making only 10 requests
//     (refactor to recursive or setTimeout if needed).
//    -You should only need to fill up your DB with all the pokemon you plan to incorporate once.
//    -The api is free to use but only 300 requests per resource per day are permitted
//     e.g. bulbasaur can be requested 300 times per day.
// 2) checkForPokemon:
//   a function that checks how many pokemon rows are in the pokemon table. If there are less rows
//   than the number of pokemon (in this case, less that the original 151 pokemon we wanted), then
//   it excutes a callback i.e. fetchFirst151Pokemon.
...
const fetchFirst151Pokemon = (callback, pokemonStoredSoFar) => {
  ...
  // you WILL get a 504 error if you try to process a large number of requests.
  // I did increments of 10.
  for (let i = pokemonStoredSoFar; i <= pokemonStoredSoFar + 10; i += 1) {
  ...
  Promise.all(arrayOfRequests)
    .then((arrOfPromises) => {
      arrOfPromises.forEach((promise) => {
        ...
        pokemonObj.baseSpecialAttack = promise.data.stats[2].base_stat;
        pokemonObj.baseSpecialDefense = promise.data.stats[1].base_stat;
        pokemonObj.baseSpeed = promise.data.stats[0].base_stat;
